...
impl HomePage {
        ...
        let cards: Element<'_, Message> = if self.recommended_games.is_empty() {
            container(
                text("Your library is empty — head to Search to find some games!")
                    .size(16)
                    .style(|_: &Theme| iced::widget::text::Style {
                        color: Some(Color::from_rgb8(140, 140, 140)),
                    }),
            )
            .width(Length::Fill)
            .padding(20)
    ...
    fn view_deals(&self) -> Element<'_, Message> {
        ...
        let deal_card: Element<'_, Message> =
            if self.deals.is_empty() {
                container(text("No deals at the moment.").size(16).style(|_: &Theme| {
                ...
                let store_badge = container(text(store_name).size(12).style(|_: &Theme| {
                    iced::widget::text::Style {
                        color: Some(Color::WHITE),
                    }
                }))
                ...
                let prev_btn: Element<'_, Message> = button(text("‹").size(28).style(
                ...
                let next_btn: Element<'_, Message> = button(text("›").size(28).style(
                    |_: &Theme| iced::widget::text::Style {
                        color: Some(Color::WHITE),
                    },
                ))
                ...
                container(
                    column![card_row, Space::new().height(16), bottom_row].width(Length::Fill),
                )
                .width(Length::Fill)
                .padding(30)
                .style(|_: &Theme| container::Style {
