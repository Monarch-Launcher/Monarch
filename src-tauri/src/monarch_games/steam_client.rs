...
/// Downloads and installs SteamCMD on users computer.
pub async fn download_and_install() -> Result<()> {
    steam::install_steamcmd().await.with_context(|| "steam_client::download_and_install() -> ")?;
    steam::steamcmd_command(vec!["+set_steam_guard_code"]).await.with_context(|| "steam_client::download_and_install() -> ")
}
...
pub async fn launch_cmd_game(id: &str) -> Result<()> {
    ...
    steam::steamcmd_command(args).await.with_context(|| "steam_client::launch_cmd_game() -> ")
}
...
pub async fn download_game(name: &str, id: &str) -> Result<MonarchGame> {
    ...
    steam::steamcmd_command(command).await.with_context(|| "steam_client::download_game() -> ")?;
...
pub async fn uninstall_game(id: &str) -> Result<()> {
    ...
    steam::steamcmd_command(command).await.with_context(|| "steam_client::uninstall_game() -> ")
}
