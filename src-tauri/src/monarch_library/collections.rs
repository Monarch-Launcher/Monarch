...
pub fn get_collections() -> Result<Value> {
    ...
    match fs::File::open(&path) {
        Ok(file) => {
            return serde_json::from_reader(file).context("collections::get_collections() failed! Failed to parse file content to json! Possibly reading an empty file. | Err")
...
fn write_collection_changes(collections: Value) -> Result<()> {
    let path: PathBuf = get_collections_json_path().with_context(|| 
        -> String {format!("collections::write_collection_changes() failed! Cannot get path to collections.json! | Err")})?;
